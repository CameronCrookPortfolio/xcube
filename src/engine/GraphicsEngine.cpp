...
void GraphicsEngine::drawRectf(const Rectangle2f& rect) { //new function to draw a Rectangle2f (float values)
	SDL_RenderDrawRect(renderer, &rect.getSDLRect());
}
...
void GraphicsEngine::drawLinef(const Point2f& p0, const Point2f& p1) { //cc
	SDL_RenderDrawLine(renderer, p0.x, p0.y, p1.x, p1.y);
}
